...
// <hpc@informatik.uni-mainz.de> SPDX-FileCopyrightText: 2019-2026 Robin Kobus
//
...
gossip::transfer_plan_t parse_plan(const char *filename) {
  ...
  if (it != json_plan.end()) {
    ...
    if (transfer_sequences.back().size() - 1 != num_steps)
      std::cerr << "transfer sequence length does not match num steps!"
                << std::endl;
  }
