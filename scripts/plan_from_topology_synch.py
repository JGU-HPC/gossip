...
for steps in range(1, max_steps + 1):
    ...
    for i in range(len(conservation_constraints)):
        conservation_constraints[i] = solver.Constraint(
            in_out_bounds[i], in_out_bounds[i]
        )
        ...
        for src in range(num_gpus):
            for trg in range(num_gpus):
                ...
                for m in range(multiplicity):
                    ...
                    edge_constraint = solver.Constraint(
                    ...
                    for c in range(num_commodities):
                        ...
                        conservation_constraints[
                            step * flows_per_gpu + src * num_commodities + c
                        ].SetCoefficient(flow, 1)
                        # incoming flow at trg
                        conservation_constraints[
                            (step + 1) * flows_per_gpu + trg * num_commodities + c
                        ].SetCoefficient(flow, -1)
    ...
    if status != solver.OPTIMAL:
        ...
        max_usage = np.max(
            count_usage - np.eye(num_gpus) * num_gpus, axis=(1, 2)
        ).astype(int)
        ...
        steps_expanded = [
            np.zeros((m + 1, num_gpus, num_gpus), dtype=bool) for m in max_usage
        ]
        plan_expanded = []
