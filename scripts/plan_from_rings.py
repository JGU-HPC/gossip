...
parser.add_argument(
    ...
    type=int,
    default=0,
    help="source for scatter or target for gather",
)
...
def make_paths(ring, src, forward=True, wait=True):
    ...
    for i in range(half_num_gpus):
        ...
        path = list(path)
        if wait:
            wait_steps = (half_num_gpus * (half_num_gpus + 1) // 2) - (
                (half_num_gpus - i) * (half_num_gpus - i + 1) // 2
            )
...
def make_broadcast_plan(src, chunks_per_ring):
    ...
    length = len(rings[0])
    path_length = (
        len(rings[0]) + chunks_per_ring - 1 + (len(rings) - 1) * chunks_per_ring
    )
    ...
    for r, ring in enumerate(rings):
        ...
        for c in range(chunks_per_ring):
            ...
            path = (
                [path[0]] * r * chunks_per_ring
                + path
                + [path[-1]] * (len(rings) - r - 1) * chunks_per_ring
            )
            plan.append(path)
            chunks.append(r * chunks_per_ring * 2 + c)
        # reverse
