...
void all2all(gossip::transfer_plan_t transfer_plan,
	...
	if (transfer_plan.valid()) {
		...
		run_multisplit_all2all<data_t>(
		    context, all2all, multisplit, point2point, batch_size, batch_size_secure);
...
template<typename data_t>
void all2all_async(gossip::transfer_plan_t transfer_plan,
                   const size_t            batch_size,
                   const size_t            batch_size_secure) {
...
template<typename data_t>
void scatter_gather(gossip::transfer_plan_t scatter_plan,
	...
	if (scatter_plan.valid() && gather_plan.valid()) {
		...
		run_multisplit_scatter_gather<data_t>(
		    context, point2point, multisplit, scatter, gather, main_gpu, batch_size, batch_size_secure);
...
template<typename data_t>
void broadcaster(gossip::transfer_plan_t transfer_plan,
                 const size_t            batch_size,
                 const size_t            batch_size_secure) {
	...
	if (transfer_plan.valid()) {
		...
		run_multisplit_broadcast<data_t>(
		    context, point2point, multisplit, broadcast, batch_size, batch_size_secure);
	...
	auto cli =
	    (((((command("all2all").set(selected, mode::all2all) |
	       ...
	       (command("scatter_gather").set(selected, mode::scatter_gather),
	        value("scatter plan", scatter_plan_file),
	        value("gather plan", gather_plan_file))),
	      option("--size", "-s") & value("size", data_size) % "data size (bytes log2) [default: 28]",
	      option("--memory-factor") &
	          value("factor", security_factor) % "memory security factor [default: 1.5]") |
	...
	if (parse(argc, argv, cli)) {
		...
		switch (selected) {
			...
			case mode::scatter_gather:
				...
				scatter_gather<data_t>(parse_plan(scatter_plan_file.c_str()),
				                       parse_plan(gather_plan_file.c_str()),
				                       data_size,
				                       data_size_secure);
				break;
			case mode::help:
				std::cout << make_man_page(cli, "execute")
				                 .prepend_section("DESCRIPTION",
				                                  "    test gossip on uniformly distributed data");
				break;
		}
	} else {
