...
__global__ void memset_kernel(data_t* data, size_t capacity, const data_t value) {
	for (size_t thid = blockDim.x * blockIdx.x + threadIdx.x; thid < capacity;
...
template<typename data_t>
void memset_all(gossip::context_t&         context,
...
__global__ void generate_data(value_t* data, index_t const length, index_t const offset) {
...
template<typename data_t>
void generate_all(gossip::context_t&         context,
                  std::vector<data_t*>&      data,
                  const std::vector<size_t>& lengths) {
	gpu_id_t num_gpus = context.get_num_devices();
	for (gpu_id_t gpu = 0; gpu < num_gpus; ++gpu) {
...
template<typename value_t, typename index_t, typename gpu_id_t, typename funct_t>
__global__ void validate(value_t const* const data,
...
template<typename data_t, class T1, class T2, class T3, class T4>
void run_multisplit_all2all(T1&          context,
                            T2&          all2all,
                            T3&          multisplit,
                            T4&          point2point,
	...
	std::cout << "INFO: " << sizeof(data_t) * batch_size * num_gpus << " bytes (all2all)"
	          << std::endl;
	...
	{
		helpers::GpuTimer gtimer(
		    context.get_streams(0)[0], "malloc_devices", context.get_device_id(0), std::cout);
		for (gpu_id_t gpu = 0; gpu < num_gpus; ++gpu) {
			cudaSetDevice(context.get_device_id(gpu));
			helpers::device::exit_at_cuda_error();
			cudaMalloc(&srcs[gpu], sizeof(data_t) * mems_lens[gpu]);
			helpers::device::exit_at_cuda_error();
			cudaMalloc(&dsts[gpu], sizeof(data_t) * mems_lens[gpu]);
			helpers::device::exit_at_cuda_error();
		}
	}
	...
	{
		helpers::GpuTimer gtimer(
		    context.get_streams(0)[0], "zero_gpu_buffers", context.get_device_id(0), std::cout);
	...
	{
		helpers::GpuTimer gtimer(
		    context.get_streams(0)[0], "init_data", context.get_device_id(0), std::cout);
	...
	{
		helpers::GpuTimer gtimer(
		    context.get_streams(0)[0], "multisplit", context.get_device_id(0), std::cout);
	...
	{
		helpers::GpuTimer gtimer(
		    context.get_streams(0)[0], "all2all", context.get_device_id(0), std::cout);
	...
	{
		helpers::GpuTimer gtimer(
		    context.get_streams(0)[0], "validate", context.get_device_id(0), std::cout);
...
template<typename data_t, class T1, class T2, class T3, class T4>
void run_multisplit_all2all_async(T1&          context,
                                  T2&          all2all,
                                  T3&          multisplit,
                                  T4&          point2point,
                                  const size_t batch_size,
                                  const size_t batch_size_secure) {
	const gpu_id_t num_gpus = context.get_num_devices();
	std::cout << "INFO: " << sizeof(data_t) * batch_size * num_gpus << " bytes (all2all_async)"
	          << std::endl;
	...
	{
		helpers::GpuTimer gtimer(
		    context.get_streams(0)[0], "malloc_buffers", context.get_device_id(0), std::cout);
		for (gpu_id_t gpu = 0; gpu < num_gpus; ++gpu) {
			cudaSetDevice(context.get_device_id(gpu));
			helpers::device::exit_at_cuda_error();
	...
	{
		helpers::GpuTimer gtimer(
		    context.get_streams(0)[0], "reset_buffers", context.get_device_id(0), std::cout);
		memset_all(context, dsts, mems_lens, data_t(0));
	...
	{
		helpers::GpuTimer gtimer(
		    context.get_streams(0)[0], "all2all_async", context.get_device_id(0), std::cout);
...
template<typename data_t, class T1, class T2, class T3, class T4, class T5>
void run_multisplit_scatter_gather(T1&          context,
                                   T2&          point2point,
                                   T3&          multisplit,
                                   T4&          scatter,
                                   T5&          gather,
	...
	{
		helpers::GpuTimer gtimer(
		    context.get_streams(0)[0], "scatter", context.get_device_id(0), std::cout);
		scatter.execAsync(
		    srcs[main_gpu], mems_lens[main_gpu], dsts, mems_lens, bufs, bufs_lens, table[main_gpu]);
		scatter.sync();
		helpers::device::exit_at_cuda_error();
	}
	...
	{
		helpers::GpuTimer gtimer(
		    context.get_streams(0)[0], "validate_scatter", context.get_device_id(0), std::cout);
		for (gpu_id_t gpu = 0; gpu < num_gpus; gpu++) {
			cudaSetDevice(context.get_device_id(gpu));
			validate<<<256, 1024, 0, context.get_streams(gpu)[0]>>>(
	...
	{
		helpers::GpuTimer gtimer(
		    context.get_streams(0)[0], "realloc_buffers", context.get_device_id(0), std::cout);
		for (gpu_id_t gpu = 0; gpu < num_gpus; ++gpu) {
	...
	{
		helpers::GpuTimer gtimer(
		    context.get_streams(0)[0], "reset_buffers_again", context.get_device_id(0), std::cout);
		memset_all(context, dsts, mems_lens, data_t(0));
		memset_all(context, bufs, bufs_lens, data_t(0));
		context.sync_all_streams();
		helpers::device::exit_at_cuda_error();
	...
	{
		helpers::GpuTimer gtimer(
		    context.get_streams(0)[0], "gather", context.get_device_id(0), std::cout);
		gather.execAsync(
		    srcs, mems_lens, dsts[main_gpu], mems_lens[main_gpu], bufs, bufs_lens, table[main_gpu]);
		gather.sync();
		helpers::device::exit_at_cuda_error();
	}
	...
	{
		helpers::GpuTimer gtimer(
		    context.get_streams(0)[0], "validate_gather", context.get_device_id(0), std::cout);
...
template<typename data_t, class T1, class T2, class T3, class T4>
void run_multisplit_broadcast(T1&          context,
                              T2&          point2point,
                              T3&          multisplit,
                              T4&          broadcast,
                              const size_t batch_size,
	...
	{
		helpers::GpuTimer gtimer(
		    context.get_streams(0)[0], "broadcast", context.get_device_id(0), std::cout);
	...
	{
		helpers::GpuTimer gtimer(
		    context.get_streams(0)[0], "validate_broadcast", context.get_device_id(0), std::cout);
		for (gpu_id_t gpu = 0; gpu < num_gpus; gpu++) {
			cudaSetDevice(context.get_device_id(gpu));
			for (gpu_id_t part = 0; part < num_gpus; part++)
