...
namespace gossip {
	...
	template<typename value_t, typename index_t, typename cnter_t, typename funct_t, typename desir_t>
	__global__ void binary_split(value_t* src,
	                             value_t* dst,
	                             index_t  len,
	                             cnter_t* counter,
	                             funct_t  part_hash,
	...
	template<typename key_t,
	         ...
	         typename funct_t,
	         typename desir_t>
	__global__ void binary_split(key_t*   src_keys,
	                             value_t* src_vals,
	                             key_t*   dst_keys,
	                             value_t* dst_vals,
	                             index_t  len,
	...
	public:
		...
		template<typename value_t, typename index_t, typename table_t, typename funct_t>
		bool execAsync(const std::vector<value_t*>&       srcs,
			...
			for (gpu_id_t gpu = 0; gpu < get_num_devices(); ++gpu) {
				cudaSetDevice(context->get_device_id(gpu));
				cudaMemsetAsync(counters_device[gpu], 0, sizeof(cnter_t), context->get_streams(gpu)[0]);
			}
			helpers::device::exit_at_cuda_error();
			...
			for (gpu_id_t gpu = 0; gpu < get_num_devices(); ++gpu) {
				...
				for (gpu_id_t part = 0; part < get_num_devices(); ++part) {
					...
					binary_split<<<256, 1024, 0, context->get_streams(gpu)[0]>>>(
					    srcs[gpu], dsts[gpu], srcs_lens[gpu], counters_device[gpu], functor, part);
			...
			for (gpu_id_t gpu = 0; gpu < get_num_devices(); ++gpu)
				for (gpu_id_t part = 0; part < get_num_devices(); ++part)
					partition_table[gpu][part] =
					    (part == 0) ? counters_host[gpu][part]
					                : counters_host[gpu][part] - counters_host[gpu][part - 1];

			return true;
		}

		template<typename key_t, typename value_t, typename index_t, typename table_t, typename funct_t>
		bool execAsync(const std::vector<key_t*>&         src_keys,
			...
			for (gpu_id_t gpu = 0; gpu < get_num_devices(); ++gpu) {
				...
				for (gpu_id_t part = 0; part < get_num_devices(); ++part) {
					...
					binary_split<<<256, 1024, 0, context->get_streams(gpu)[0]>>>(src_keys[gpu],
					                                                             src_vals[gpu],
					                                                             dst_keys[gpu],
					                                                             dst_vals[gpu],
					                                                             src_lens[gpu],
