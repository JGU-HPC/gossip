...
namespace gossip {
	...
	public:
		static void verify_plan(transfer_plan_t& plan) {
			...
			std::vector<std::vector<size_t>> completeness(plan.num_gpus(),
			                                              std::vector<size_t>(plan.num_gpus()));
			for (const auto& sequence : plan.transfer_sequences()) {
			...
			for (gpu_id_t src = 0; src < plan.num_gpus(); ++src) {
				for (gpu_id_t trg = 0; trg < plan.num_gpus(); ++trg) {
					valid &=
					    check(completeness[src][trg] == plan.num_chunks(), "transfer plan is incomplete.");
				}
			}
