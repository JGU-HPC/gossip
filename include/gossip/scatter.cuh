...
namespace gossip {
	...
	public:
		scatter_t(const context_t& context_, const gpu_id_t main_gpu_)
		    : context(&context_),
		...
		scatter_t(const context_t& context_, const transfer_plan_t& transfer_plan_)
	...
	private:
		...
		transfer_handler<table_t> makeTransferHandler(const std::vector<table_t>& send_counts,
			...
			for (gpu_id_t part = 0; part < get_num_devices(); ++part) {
				...
				src_displacements[main_gpu][part + 1] =
				    send_counts[part] + src_displacements[main_gpu][part];
			}

			std::vector<std::vector<size_t>> trg_displacements(get_num_devices() + 1,
			                                                   std::vector<size_t>(get_num_devices()));

			std::vector<std::vector<table_t>> all_send_counts(get_num_devices(),
			                                                  std::vector<table_t>(get_num_devices()));
			for (gpu_id_t part = 0; part < get_num_devices(); ++part) {
				all_send_counts[main_gpu][part] = send_counts[part];
			}

			transfer_handler<table_t> transfers(
			    context, src_displacements, trg_displacements, all_send_counts, num_phases, num_chunks);
	...
	public:
		 ...
		 */
		template<typename table_t>
		const std::vector<size_t> calcBufferLengths(const std::vector<table_t>& send_counts,
		                                            bool                        verbose = false) const {
		...
		bool execAsync(value_t*                     src,
			...
			if (!check_size(send_counts, dsts_lens))
				return false;
