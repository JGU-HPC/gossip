...
namespace gossip {
	...
	public:
		...
		std::vector<value_t*> alloc_device(const std::vector<index_t>& lens,
			...
			for (gpu_id_t gpu = 0; gpu < get_num_devices(); ++gpu) {
				...
				if (zero)
					cudaMemsetAsync(data[gpu], 0, sizeof(value_t) * lens[gpu], context->get_streams(gpu)[0]);
			}
		...
		template<typename value_t, typename index_t>
		std::vector<value_t*> alloc_host(const std::vector<index_t>& lens,
		                                 const bool                  zero = true) const {
		...
		bool free_device(std::vector<value_t*>& data) const {
			...
			if (!check(data.size() == get_num_devices(), "data size does not match number of gpus."))
				return false;
