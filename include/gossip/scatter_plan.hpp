...
namespace gossip {
	...
	public:
		static void verify_plan(transfer_plan_t& plan) {
			...
			valid &= check(plan.main_gpu() != gpu_id_t(-1), "main gpu not set in plan.");

			valid &= check(plan.num_steps() >= 1, "planned sequence must be at least of length 2.");
			...
			for (const auto& sequence : plan.transfer_sequences()) {
				valid &=
				    check(sequence.seq.front() == plan.main_gpu(), "all sequences must have same source.");
			}
			...
			for (gpu_id_t trg = 0; trg < plan.num_gpus(); ++trg) {
				valid &= check(completeness[trg] == plan.num_chunks(), "transfer plan is incomplete.");
			}
