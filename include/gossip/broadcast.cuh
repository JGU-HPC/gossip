...
namespace gossip {
	...
	public:
		broadcast_t(const context_t& context_, const gpu_id_t main_gpu_)
		    : context(&context_),
		...
		broadcast_t(const context_t& context_, const transfer_plan_t& transfer_plan_)
	...
	private:
		struct transfer {
			...
			transfer(cudaEvent_t* event_before, cudaEvent_t* event_after)
			...
			void set_pos(const size_t src_pos_, const size_t trg_pos_, const size_t len_) {
				src_pos = src_pos_;
				trg_pos = trg_pos_;
				len     = len_;
			}
			...
			void show(const gpu_id_t src_gpu, const gpu_id_t trg_gpu) const {
				std::cout << "src:" << uint32_t(src_gpu) << ", pos:" << src_pos
				          << ", trg:" << uint32_t(trg_gpu) << ", pos:" << trg_pos << ", len:" << len
				          << std::endl;
			}
		};
		...
		struct transfer_handler {
			...
			transfer_handler(const context_t* context, const transfer_plan_t& transfer_plan)
			    : context(context), transfer_plan(transfer_plan),
			...
			gpu_id_t num_gpus() const noexcept { return transfer_plan.num_gpus(); }

			gpu_id_t num_steps() const noexcept { return transfer_plan.num_steps(); }

			gpu_id_t num_chunks() const noexcept { return transfer_plan.num_chunks(); }

			template<typename table_t>
			void calculate_offsets(const std::vector<size_t>&  src_displacements,
				...
				for (const auto& sequence : transfer_plan.transfer_sequences()) {
					...
					if (verbose)
						std::cout << "transfer from " << int(sequence.seq.front()) << " to "
						          << int(sequence.seq.back()) << std::endl;
			...
			void execute_step(const size_t                 step,
				...
				for (gpu_id_t t_src_gpu = 0; t_src_gpu < num_gpus(); ++t_src_gpu) {
					for (gpu_id_t t_trg_gpu = 0; t_trg_gpu < num_gpus(); ++t_trg_gpu) {
						...
						if (t && t->len > 0) {
							...
							const value_t* from = (t->event_before == nullptr) ? sendbuf + t->src_pos
							                                                   : recvbufs[t_src_gpu] + t->src_pos;
							value_t*       to   = recvbufs[t_trg_gpu] + t->trg_pos;

							if (t->event_before != nullptr)
								cudaStreamWaitEvent(stream, *(t->event_before), 0);
			...
			template<typename value_t>
			void execute_steps(const value_t* sendbuf, const std::vector<value_t*>& recvbufs) {
				for (size_t step = 0; step < num_steps(); ++step)
	...
	public:
		template<typename value_t,
		         typename index_t>
		bool
		execAsync(const value_t*               sendbuf,      // sendbuf resides on device_ids[main_gpu]
			...
			//     return false;
			// if (!check_transfers_size(transfers.trg_offsets, transfers.aux_offsets,
			// recvbufs_lens))
			//     return false;
