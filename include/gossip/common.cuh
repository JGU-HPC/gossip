	...
	struct transfer {
		...
		void show() const {
			std::cout << "src:" << int(src_gpu) << ", pos:" << src_pos << ", trg:" << int(trg_gpu)
			          ...
			          << ", event after:" << (event_after ? event_after : 0) << std::endl;
		}
	};
	...
	struct transfer_handler {
		...
		transfer_handler(const context_t*                         context_,
		    ...
		    : context(context_),
		      ...
		      aux_offsets(context->get_num_devices()), src_displacements(src_displacements),
		      sizes(sizes), num_phases(num_phases_), phases(num_phases), num_chunks(num_chunks_) {}
		...
		bool push_back(const std::vector<gpu_id_t>& sequence,
			...
			if (verbose)
				std::cout << "transfer from " << int(sequence.front()) << " to " << int(sequence.back())
				          << std::endl;
			...
			if (sequence.front() == sequence.back()) { // src == trg
				...
				phases[phase].emplace_back(
				    src, *src_offset, trg, *trg_offset, transfer_size, event_before, event_after);
				if (verbose)
					phases[phase].back().show();
		...
		bool execute_phase(const size_t                 phase,
		                   ...
		                   const std::vector<value_t*>& bufs) const {
			for (const transfer& t : phases[phase]) {
				...
				value_t*     from =
            (t.event_before == nullptr) ? srcs[t.src_gpu] + t.src_pos : bufs[t.src_gpu] + t.src_pos;
				value_t* to =
				    (t.event_after == nullptr) ? dsts[t.trg_gpu] + t.trg_pos : bufs[t.trg_gpu] + t.trg_pos;
