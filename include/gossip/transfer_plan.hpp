...
namespace gossip {
	...
	public:
		transfer_plan_t(const std::string                         type,
		                const gpu_id_t                            num_gpus,
		                const std::vector<std::vector<gpu_id_t>>& sequences)
		    : type_(type), num_gpus_(num_gpus), main_gpu_(gpu_id_t(-1)), num_steps_(0), num_chunks_(1),
		      valid_(false) {
			if (sequences.size())
		...
		transfer_plan_t(const std::string                         type,
		    ...
		    : type_(type), num_gpus_(num_gpus), main_gpu_(gpu_id_t(-1)), num_steps_(0),
		      num_chunks_(num_chunks), valid_(false) {
	...
	public:
		...
		gpu_id_t num_gpus() const noexcept { return num_gpus_; }

		gpu_id_t main_gpu() const noexcept { return main_gpu_; }

		void main_gpu(const gpu_id_t gpu) { main_gpu_ = gpu; }

		size_t num_steps() const noexcept { return num_steps_; }

		size_t num_chunks() const noexcept { return num_chunks_; }

		const std::vector<transfer_sequence>& transfer_sequences() const { return transfer_sequences_; }

		const std::vector<size_t>& sync_steps() { return sync_steps_; }

		void sync_steps(const std::vector<size_t>& steps) { sync_steps_ = steps; }

		bool synchronized() const noexcept { return sync_steps_.size() > 0; }
		...
		void show_plan() const {
			...
			std::cout << "INFO: Transfer " << uint32_t(num_chunks_) << " chunks in " << num_steps_
			          << " steps\n";
			...
			for (const auto& sequence : transfer_sequences_) {
				std::cout << "\tTransfer " << sequence.size << " chunks via [";
