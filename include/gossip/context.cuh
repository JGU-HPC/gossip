...
namespace gossip {
	...
	public:
		context_t(const gpu_id_t num_gpus_) {
			...
			valid = check(num_gpus_ > 0, "Invalid number of devices.");
		...
		context_t(const std::vector<gpu_id_t>& device_ids_) {
			...
			valid = check(!device_ids_.empty(), "Invalid number of device ids.");
		...
		void initialize() {
			...
			for (gpu_id_t src_gpu = 0; src_gpu < num_gpus; ++src_gpu) {
				...
				for (gpu_id_t dst_gpu = 0; dst_gpu < num_gpus; ++dst_gpu) {
					...
					if (src == dst) {
						...
						peer_status[src_gpu][dst_gpu] = status ? PEER_STATUS::FAST : PEER_STATUS::SLOW;
					}
				}
			}
			helpers::device::exit_at_cuda_error();
			...
			for (gpu_id_t src_gpu = 0; src_gpu < num_gpus; ++src_gpu) {
				...
				for (gpu_id_t dst_gpu = 0; dst_gpu < num_gpus; ++dst_gpu) {
					...
					if (src_gpu != dst_gpu) {
						if (src == dst)
							std::cout << "WARNING: device identifiers are not unique." << std::endl;
					}
					...
					if (peer_status[src_gpu][dst_gpu] == PEER_STATUS::FAST) {
						...
						if (cuerr == cudaErrorPeerAccessAlreadyEnabled)
							std::cout << "STATUS: redundant enabling of "
							          << "peer access from GPU " << src << " to GPU " << dst << " attempted."
							          << std::endl;
						else if (cuerr)
							std::cout << "CUDA error: " << cudaGetErrorString(cuerr) << " : " << __FILE__
	...
	public:
					...
					if (peer_status[src_gpu][dst_gpu] == PEER_STATUS::FAST) {
						...
						if (cuerr == cudaErrorPeerAccessNotEnabled)
							std::cout << "STATUS: redundant disabling of "
							          << "peer access from GPU " << src_gpu << " to GPU " << dst << " attempted."
							          << std::endl;
						else if (cuerr)
							std::cout << "CUDA error: " << cudaGetErrorString(cuerr) << " : " << __FILE__
							          << ", line " << __LINE__ << std::endl;
		...
		gpu_id_t get_num_devices() const noexcept { return num_gpus; }

		// return the actual device identifier of specified GPU
		gpu_id_t get_device_id(const gpu_id_t gpu) const noexcept { return device_ids[gpu]; }
		...
		bool is_valid() const noexcept { return !streams.empty() && !device_ids.empty(); }

		void print_connectivity_matrix() const {
			std::cout << "STATUS: connectivity matrix:" << std::endl;
			for (gpu_id_t src_gpu = 0; src_gpu < num_gpus; ++src_gpu)
				for (gpu_id_t dst_gpu = 0; dst_gpu < num_gpus; ++dst_gpu)
					std::cout << (dst_gpu == 0 ? "STATUS: |" : "") << uint32_t(peer_status[src_gpu][dst_gpu])
					          << (dst_gpu + 1 == num_gpus ? "|\n" : " ");
		}
	};
