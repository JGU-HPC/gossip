...
namespace gossip {
	...
	public:
		all2all_async_t(const context_t& context_)
		...
		all2all_async_t(const context_t& context_, const transfer_plan_t& transfer_plan_)
	...
	private:
		...
		transfer_handler<table_t>
		makeTransferHandler(const std::vector<std::vector<table_t>>& send_counts,
	...
	public:
		 ...
		 * @param send_counts send_counts[k][l] elements are sent from device_ids[k]
		 * to device_ids[l].
		...
		calcBufferLengths(const std::vector<std::vector<table_t>>& send_counts,
		                  bool                                     verbose = false) const {
		 ...
		 * The lenghts of the parameters have to match the context.
		 * @param srcs pointers to source arrays. srcs[k] array should reside on
		 * device_ids[k].
		 * @param srcs_len srcs_len[k] is length of srcs[k] array.
		 * @param dsts pointers to destination arrays. dsts[k] array should reside on
		 * device_ids[k].
		 * @param dsts_len dsts_len[k] is length of dsts[k] array.
		 * @param bufs pointers to buffer arrays. bufs[k] array should reside on
		 * device_ids[k].
		 ...
		 */
		template<typename value_t, typename index_t, typename table_t>
		bool execAsync(const std::vector<value_t*>&             srcs,
			...
			if (!check(dsts_lens.size() == get_num_devices(),
				...
				return false;
			if (!check(bufs.size() == get_num_devices(), "bufs size does not match number of gpus."))
				return false;
			...
			if (!check_size(transfers.aux_offsets, bufs_lens))
				return false;
			if (!check_size(transfers.trg_offsets.back(), dsts_lens))
				return false;
