...
namespace gossip {
	...
	public:
		gather_t(const context_t& context_, const gpu_id_t main_gpu_)
		    : context(&context_),
		...
		gather_t(const context_t& context_, const transfer_plan_t& transfer_plan_)
	...
	private:
		...
		transfer_handler<table_t> makeTransferHandler(const std::vector<table_t>& send_counts,
			...
			for (gpu_id_t part = 0; part < get_num_devices(); ++part) {
				...
				trg_displacements[part + 1][main_gpu] =
				    send_counts[part] + trg_displacements[part][main_gpu];
			}

			std::vector<std::vector<table_t>> sizes(get_num_devices(),
			...
			transfer_handler<table_t> transfers(
			    context, src_displacements, trg_displacements, sizes, num_phases, num_chunks);
	...
	public:
		...
		bool execAsync(const std::vector<value_t*>& srcs,
			...
			if (!check_size(send_counts, srcs_lens))
				return false;
