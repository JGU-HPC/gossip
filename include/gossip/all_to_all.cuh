...
namespace gossip {
	...
	public:
		all2all_t(const context_t& context_)
		    : context(&context_), transfer_plan(all2all::default_plan(context->get_num_devices())),
		      plan_valid(transfer_plan.valid()) {
			check(context->is_valid(), "You have to pass a valid context!");
		}

		all2all_t(const context_t& context_, const transfer_plan_t& transfer_plan_)
		    : context(&context_), transfer_plan(transfer_plan_), plan_valid(false) {
			check(context->is_valid(), "You have to pass a valid context!");
			...
			plan_valid = (get_num_devices() == transfer_plan.num_gpus()) && transfer_plan.valid();
		}
	...
	private:
		struct transfer {
			...
			transfer(const gpu_id_t src_gpu,
			         ...
			         const size_t   trg_pos,
			         const size_t   len)
			    : src_gpu(src_gpu), src_pos(src_pos), trg_gpu(trg_gpu), trg_pos(trg_pos), len(len) {}

			void show() const {
				std::cout << "src:" << int(src_gpu) << ", pos:" << src_pos << ", trg:" << int(trg_gpu)
				          << ", pos:" << trg_pos << ", len:" << len << std::endl;
			}
		};
		...
		struct transfer_handler {
			...
			transfer_handler(const context_t*                         context_,
			    ...
			    : context(context_),
			      ...
			      src_displacements(src_displacements), sizes(sizes), num_phases(num_phases_),
			      phases(num_phases), num_chunks(num_chunks_) {
				if (num_phases > 1)
					phases_offsets.resize(num_phases - 1, std::vector<size_t>(context->get_num_devices()));
			}

			bool push_back(const std::vector<gpu_id_t>& sequence, const size_t chunks = 1) {
				...
				if (num_phases == 1) {
					size_t phase = 0;
					phases[phase].emplace_back(
					    sequence[phase], src_offset, sequence[phase + 1], trg_offset, transfer_size);
				} else {
					size_t phase = 0;
					phases[phase].emplace_back(sequence[phase],
					                           src_offset,
					                           sequence[phase + 1],
					                           phases_offsets[phase][sequence[phase + 1]],
					                           transfer_size);

					for (phase = 1; phase < num_phases - 1; ++phase) {
						phases[phase].emplace_back(sequence[phase],
						                           phases_offsets[phase - 1][sequence[phase]],
						                           sequence[phase + 1],
						                           phases_offsets[phase][sequence[phase + 1]],
						                           transfer_size);
					}
			...
			bool execute_phase(const size_t                 phase,
	...
	public:
		template<typename value_t,
		         typename index_t,
		         typename table_t>
		bool execAsync(std::vector<value_t*>&      srcs,      // src[k] resides on device_ids[k]
			...
			if (!check(srcs.size() == get_num_devices(), "srcs size does not match number of gpus."))
				return false;
			if (!check(srcs_lens.size() == get_num_devices(),
			           "srcs_lens size does not match number of gpus."))
				return false;
			if (!check(dsts.size() == get_num_devices(), "dsts size does not match number of gpus."))
				return false;
			...
			for (const auto& counts : send_counts) {
				if (!check(counts.size() == get_num_devices(), "table size does not match number of gpus."))
					return false;
			}
			...
			std::vector<std::vector<size_t>> src_displacements(
			    get_num_devices(), std::vector<size_t>(get_num_devices() + 1));
			...
			std::vector<std::vector<size_t>> trg_displacements(get_num_devices() + 1,
			                                                   std::vector<size_t>(get_num_devices()));
			...
			transfer_handler<table_t> transfers(
			    context, src_displacements, trg_displacements, send_counts, num_phases, num_chunks);
			...
			for (size_t p = 0; p < num_phases - 1; ++p) {
				if (!check_size(transfers.phases_offsets[p], dsts_lens))
					return false;
			}
			if (!check_size(transfers.trg_offsets[get_num_devices()], dsts_lens))
				return false;
		...
		gpu_id_t get_num_devices() const noexcept { return context->get_num_devices(); }

		void sync() const noexcept { context->sync_all_streams(); }

		void sync_hard() const noexcept { context->sync_hard(); }

		const context_t& get_context() const noexcept { return *context; }
	};
