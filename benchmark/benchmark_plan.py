...
parser.add_argument(
    "type",
    type=str,
    help="collective type (all2all|all2all_async|scatter_gather|broadcast)",
)
parser.add_argument(
    "plan",
    type=str,
    nargs="+",
    help="JSON which specifies the communication strategy\n\
    scatter_gather requires one plan for each collective",
)
parser.add_argument(
    "--output", "-o", type=str, help="output file", default="benchmark.csv"
)
parser.add_argument(
    "--repeats", "-r", type=int, help="number of repeated executions", default=3
)
parser.add_argument(
    "--maxsize",
    type=int,
    help="maximum overall amount of data to be communicated (bytes log2)",
    default=28,
)
parser.add_argument(
    "--minsize",
    type=int,
    help="minimum overall amount of data to be communicated (bytes log2)",
    default=12,
)
    ...
    for r in range(args.repeats):
        ...
        if args.type == "scatter_gather":
            out = subprocess.check_output(
                [exe, args.type, plans[0], plans[1], "--size", str(s)]
            )
        else:
